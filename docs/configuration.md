...
- `board_manager`
  ...
  - `enable_unsafe_install: false` - set to `true` to allow installation of packages that do not pass the checksum test. This
...
- `directories` - directories used by Arduino CLI.
  - [`data:`](#directories-data) - directory used to store Boards/Library Manager index files and Boards Manager platform installations.
  - [`downloads:`](#directories-data) - directory used to stage downloaded archives during Boards/Library Manager installations.
  - [`user:`](#directories-data) - the equivalent of the Arduino IDE's ["sketchbook" directory][sketchbook directory]. Library Manager
  ...
  - `builtin.libraries` - the libraries in this directory will be available to all platforms without the need for the
    ...
    equivalent of the Arduino IDE's bundled libraries directory. `directories.builtin.libraties` does not have a default value.
- `library` - configuration options relating to Arduino libraries.
  - `enable_unsafe_install: false` - set to `true` to enable the use of the `--git-url` and `--zip-file` flags with
...
- `locale` - the language used by Arduino CLI to communicate to the user, the parameter is the language identifier in
  the standard POSIX format `<language>[_<TERRITORY>[.<encoding>]]` (for example `it` or `it_IT`, or `it_IT.UTF-8`). `locale` does have a default value
- `logging` - configuration options for Arduino CLI's logs.
  - `file` - path to the file where logs will be written. `logging.file` does not have a default value
  - `format: text` - output format for the logs. Allowed values are `text` or `json`.
  - `level: info` - messages with this level and above will be logged. Valid levels are: `trace`, `debug`, `info`, `warn`,
    `error`, `fatal`, `panic`.
...
- `output` - settings related to text output.
  - `no_color: false` - ANSI color escape codes are added by default to the output. Set to `true` to disable colored text
    output.
- `sketch` - configuration options relating to [Arduino sketches][sketch specification].
  - `always_export_binaries: false` - set to `true` to make [`arduino-cli compile`][arduino-cli compile] always save binaries
...
- `updater` - configuration options related to Arduino CLI updates
  - `enable_notification: true` - set to `false` to disable notifications of new Arduino CLI releases, defaults to `true`
- `build_cache` configuration options related to the compilation cache
  - [`path`](build_cache-path) - the path to the build cache.
  - `extra_paths` - a list of paths to look for precompiled artifacts if not found on `build_cache.path` setting. `build_cache.extra_paths` does have a default value
  - `compilations_before_purge: 10` - interval, in number of compilations, at which the cache is purged, defaults to `10`.
    When `0` the cache is never purged.
  - `ttl: 720h0m0s` - cache expiration time of build folders. If the cache is hit by a compilation the corresponding build files
...
- `network` - configuration options related to the network connection.
  - `proxy` - URL of the proxy server. `network.proxy` does have a default value
  - `connection_timeout: 1m0s` - network connection timeout, the value format must be a valid input for
  ...
  - `cloud_api.skip_board_detection_calls` - if set to `true` it will make the Arduino CLI skip network calls to Arduino
    Cloud API to help detection of an unknown board. `network.cloud_api.skip_board_detection_calls` does have a default value

### Default directories

The default directories is os-dependant.

<a id="directories-data"></a>
`directories.data` default's to:

  - `{HOME}/.arduino15` on Linux (and other Unix-based OS)
  - `{HOME}/AppData/Local/Arduino15` on Windows
  - `{HOME}/Library/Arduino15` on MacOS

<a id="directories-dowload"></a>
...
<a id="directories-user"></a>
- `directories.user` default's to:
  - `{HOME}/Arduino` on Linux (and other Unix-based OS)
  - `{DOCUMENTS}/Arduino` on Windows
  - `{HOME}/Documents/Arduino` on MacOS
 
<a id="build_cache-path"></a>
- `build_cache.path` default's to:
